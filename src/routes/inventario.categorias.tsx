...
function CategoriesPage() {
  ...
  const filteredCats = useMemo(() => {
    ...
    return initialCategories
      .filter((c) => c.active === (status === "active"))
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={status} onValueChange={(v) => { setStatus(v as "active" | "inactive"); setCatPage(1); }}>
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                ...
                <div className="flex gap-1 opacity-0 transition group-hover:opacity-100">
                  <Button size="icon" variant="ghost" className="h-8 w-8 text-brand hover:bg-brand/10" onClick={(e) => { e.stopPropagation(); setEdit(c); }}>
                    <Pencil className="h-4 w-4" />
                  </Button>
                  <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:bg-destructive/10" onClick={(e) => { e.stopPropagation(); setDel(c); }}>
              ...
              <div className="mt-3 flex items-center justify-between text-xs">
                ...
                {!c.active && <span className="rounded-full bg-muted px-2 py-0.5 text-muted-foreground">Inactiva</span>}
              </div>
            </Card>
          );
        })}
        {catSlice.length === 0 && (
          <div className="col-span-full py-10 text-center text-muted-foreground">No hay categorías para mostrar.</div>
        )}
      </div>
