        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="space-y-4 min-w-0">
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-center">Entreg.</TableHead>
            ...
            <div className="rounded-xl border border-border bg-muted/30 p-3">
              ...
              {sale.discountAmount ? (
                <Row
                  label={<span className="inline-flex items-center gap-1"><Percent className="h-3.5 w-3.5" /> Descuento ({sale.discountPercent}%)</span>}
          ...
          {/* Columna derecha: Pagos + Factura + Remitos */}
          <div className="space-y-4 min-w-0">
                ...
                <div className="flex items-center justify-between rounded-xl border border-border bg-card px-3 py-2">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-sm font-semibold text-navy">Factura {sale.invoiceNumber}</div>
                  ...
                  {sale.deliveries.map((d) => (
                    <div key={d.id} className="rounded-xl border border-border bg-card p-3">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-2">
                          <Truck className="h-4 w-4 shrink-0 text-navy" />
                          <div className="min-w-0">
                            <div className="truncate text-sm font-semibold text-navy">{d.id}</div>
                            <div className="text-xs text-muted-foreground">{d.date}</div>
                          </div>
                        </div>
                        <Button size="sm" variant="outline" className="h-8 shrink-0" onClick={() => downloadRemito(d.id)}>
                          <Download className="mr-1 h-3.5 w-3.5" /> Descargar
                        </Button>
                      </div>
